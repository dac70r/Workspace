...
reg [7:0] tx_buffer_1 = 8'b1000_0001;
...
always @ (posedge system_clock) begin
	...
	if((negedge_4mhz == posedge_4mhz) && (prev_negedge_4mhz!=negedge_4mhz || prev_posedge_4mhz!=posedge_4mhz)) begin
		...
		begin		
				...
				ADC_SPI_MOSI_temp <= tx_buffer_1[7-tx_index];
				tx_index <= tx_index + 1; end
		end

		else begin 
			if (tx_index == 8) begin tx_index <= 0; stop_tx <= 1; end
			ADC_SPI_SCLK_temp <= 'd0; ADC_SPI_MOSI_temp <= 'd0;
		end
	end
	...
	// Detects the Falling Edge of the SPI_Clock
		...
		// Condition: While start_tx is TRUE, while tx_index==9
		if(start_tx || tx_index==8) begin 
  ...
  always @(posedge system_clock, negedge rst_n) begin
	...
	always @ (*) begin
		case (presentState)
			...
			WAIT_FOR_RDWR: begin
					if(1) begin nextState = WR_ASSERT_CS; end
			...
			WR_PROCESS: begin 
					if(tx_index == 8) begin nextState = WR_WAIT_FALLING_1MHZ; end
			...
			DELAY_1: begin
					...
					if(delay_counter >= 'd10) // Optional delay
						nextState = WAIT_FOR_RDWR;
