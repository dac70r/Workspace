...
#include <spi.h>
//
// Configure Register settings - NOTE: These values can be change, but FIXED
// mode is recommended!
//
...
// This example code is only targeted at asynchronous slave mode with hamming
...
//#ifdef WORD_LENGTH_32BIT
#define WORD_LENGTH_32BIT_ZERO_PADDED

extern uint8_t adcReadyFlag;
// M2 -> Tied to GND
...
// Read Data Functions
adc_data_struct ADS131A0xReadData(uint8_t NumBytes, uint8_t DataByteStartNum);
...
#define OPCODE_READY ((uint16_t)0xFF04)
 ...
 * 1   |   Bit 0   |
 * -------------------------------------------------------------------------------------------------
 * |     0     |   F_OPC   |   F_SPI   |  F_ADCIN  |   F_WDT   |  F_RESYNC |
 * F_DRDY  |  F_CHECK  |
 * -------------------------------------------------------------------------------------------------
 */
 ...
 * 1   |   Bit 0   |
 * -------------------------------------------------------------------------------------------------
 * |   WDT_EN  |  CRC_MODE |       DNDLY[1:0]      |      HIZDLY[1:0]      |
 * FIXED   |   CRC_EN  |
 * -------------------------------------------------------------------------------------------------
 */
...
/* CLK1 edited value */
#define CLK1_EDITED ((uint8_t)0x02)
...
/* CLK2 edited value */
#define CLK2_EDITED ((uint8_t)0x4F)
...
// NOTE: These macros can be used in logical expressions for checking known
// register values. These macros will take on the value from the last register
// read/write operation, or the default register value if no register operations
// have occurred or the device is reset.

#define CRC_EN \
    ((bool)(getRegisterValue(D_SYS_CFG_ADDRESS) & D_SYS_CFG_CRC_EN_MASK))
#define CRC_MODE \
    ((bool)(getRegisterValue(D_SYS_CFG_ADDRESS) & D_SYS_CFG_CRC_MODE_MASK))
#define FIXED \
    ((bool)(getRegisterValue(D_SYS_CFG_ADDRESS) & D_SYS_CFG_FIXED_MASK))
