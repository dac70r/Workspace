...
#define WORD_LENGTH (WORD_LENGTH_BITS >> 3) // 4 bytes
...
static uint8_t dataTx[6 * WORD_LENGTH] = {0}; // 20 elements
static uint8_t dataRx[6 * WORD_LENGTH] = {0}; // 20 elements
...
// Define ADC Ready Flag
uint8_t adcReadyFlag;
...
//!
//!  NOTE: The adcStartup() function should be called after resetting the
//!  device.
//!
//! \return None.
//
...
//     // Minimum nRESET pulse width (tSRLRST) equals 2,048 CLKIN periods (1 ms
//     @ 2.048 MHz) HAL_Delay(2);
...
void adcStartup(void) {
    ...
    /* (INTERNAL) Initialize internal 'registerMap' array with device default
     * settings */
    restoreRegisterDefaults();
    ...
    /* (REQUIRED) Always send a NULL command first to establish SPI
     * communication */
    uint16_t isDeviceReady = ADS131A0xSendCmd(OPCODE_NULL);

    HAL_Delay(50);

    /* (OPTIONAL) Verify READY response */
	isDeviceReady = ADS131A0xSendCmd(OPCODE_NULL);

    if (OPCODE_READY != isDeviceReady) {
    	char error_message[30] = "Error, Init Failed!\n";
        while(1){
        	//HAL_UART_Transmit(&huart4, (uint8_t*)error_message, sizeof(error_message),100);
        };
    }

    /* (REQUIRED) Send UNLOCK to exit 'READY' state  */
    uint16_t isUnlocked = ADS131A0xSendCmd(0x0655);

    /* Verify device response - Receive 0x0655 */
	if (OPCODE_UNLOCK != isUnlocked) {
	   char error_message[30] = "Error, Failed to Unlock!\n";
	   uint16_t isUnlocked = ADS131A0xSendCmd(0x0655);
	   //HAL_UART_Transmit(&huart3, (uint8_t*)error_message, sizeof(error_message),100);
	   while(1);
	}

	/* (OPTIONAL) Read Device ID */
	uint16_t isDeviceID = ADS131A04xReadDeviceID();

	/* Configure Device Register Settings */
    ...
    // Write to D_SYS_CFG_ADDRESS (write 0x003e)
    uint16_t ReadData = ADS131A0xWriteRegister(D_SYS_CFG_ADDRESS, regVal);

    // Configure data rate -- read
    ReadData = ADS131A0xWriteRegister(CLK1_ADDRESS, CLK1_CLK_DIV_2);
    ReadData = ADS131A0xWriteRegister(CLK2_ADDRESS, CLK2_ICLK_DIV_2 | CLK2_OSR_2048);

    // Enable all ADC channels -- read 0x200f
    ReadData = ADS131A0xWriteRegister(ADC_ENA_ADDRESS, ADC_ENA_ENA_ALL_CH_PWUP);

    /* (REQUIRED) Wake Device -- read 0x0033
     * communication */
    uint16_t isWakeUp = ADS131A0xSendCmd(OPCODE_WAKEUP);

    // Verify device response - Has Device Been Successfully Woken Response is 0x0033
	if (OPCODE_WAKEUP != isWakeUp) {
	   char error_message[30] = "Error, Failed to Wake Device!\n";
	   //HAL_UART_Transmit(&huart3, (uint8_t*)error_message, sizeof(error_message),100);
	   while(1);
	}

	/* (REQUIRED) Lock Registers */
	// Verify Lock Status - read 0x0055
	uint16_t isLocked = ADS131A0xSendCmd(OPCODE_LOCK);
	if (OPCODE_LOCK != isLocked) {
	   char error_message[30] = "Error, Failed to Lock!\n";
	   //HAL_UART_Transmit(&huart3, (uint8_t*)error_message, sizeof(error_message),100);
	   while(1);
	}
...
uint8_t readSingleRegister(uint8_t address) {
    ...
    HAL_SPI_TransmitReceive(&hspi6, dataTx, dataRx, cmdByteLength, 1000);
...
void writeSingleRegister(uint8_t address, uint8_t data) {
    ...
    // (OPTIONAL) Enforce certain register settings when writing to the
    // D_SYS_CFG register to avoid having to write code that supports multiple
    // device modes. For example...
    ...
    // NOTE: This ensures that the NULL command in the next frame is sent
    // correctly.
...
//!
//! \param dataStruct pointer to data structure where results from reading data
//! will be placed
//!
//! NOTE: This is currently the only function in this example that verifies if
//! the CRC word on DOUT is correct.
//!
...
// NOTE: The following CRC words are hard-coded based on word length, fixed
// mode, and CRC mode...
#if defined SET_FIXED && !defined SET_CRC_MODE  // Only the command word is
...
#else  // FIXED = 0 OR (FIXED = 1 and CRC_MODE = 1) -> All device words are
       // included in CRC calculation
    ...
    dataStruct->response = dataRx[0]<<8 | dataRx[1]; //COMBINE_BYTES(dataRx[0], dataRx[1]);
    dataStruct->channel0 = (dataRx[4] << 16) | (dataRx[5] << 8) | (dataRx[6]); //signExtend(&dataRx[1 * WORD_LENGTH]);
    dataStruct->channel1 = (dataRx[8] << 16) | (dataRx[9] << 8) | (dataRx[10]); //signExtend(&dataRx[2 * WORD_LENGTH]);
    dataStruct->channel2 = (dataRx[12] << 16) | (dataRx[13] << 8) | (dataRx[14]); //signExtend(&dataRx[3 * WORD_LENGTH]);
    dataStruct->channel3 = (dataRx[16] << 16) | (dataRx[17] << 8) | (dataRx[18]); // signExtend(&dataRx[4 * WORD_LENGTH]);
    dataStruct->crc =
        COMBINE_BYTES(dataRx[5 * WORD_LENGTH], dataRx[5 * WORD_LENGTH + 1]);

#ifdef CRC_EN
    uint16_t crcWordOut = calculateCRC(&dataRx[0], 6 * WORD_LENGTH, 0xFFFF);
#else
    // (OPTIONAL) Ignore CRC error checking (CRC = 0x00 -> indicates a valid SPI
    // frame)
...
void shift_left_and_carry(uint8_t *array, size_t length) {
    uint8_t carry = 0;
    for (size_t i = 0; i < length; i++) {
        uint8_t new_carry = (array[i] & 0x80) >> 7;  // Get the MSB before the shift
        array[i] = ((array[i] << 1) & 0xFE) | carry; // Shift left, mask out the MSB, and add the previous carry
        carry = new_carry;
    }
}
...
//*****************************************************************************
//
//! Sends the specified SPI command to the ADC (NULL, RESET, STANDBY, or
//! WAKEUP).
//!
...
uint16_t sendCommand(uint16_t opcode) {
    ...
    // SPI Transaction
    result = HAL_SPI_TransmitReceive(&hspi6, dataTx, dataRx, cmdByteLength, 1000);
    if (result != HAL_OK){
    	while(1);
...
//!
//! \return boolean to indicate if registers are locked (0 = unlocked; 1 =
//! locked)
//
//*****************************************************************************
...
bool unlockRegisters(void) {
    ...
    dataRx[0] = UPPER_BYTE(OPCODE_NULL); // reset value to 0x0000 first
    dataRx[1] = LOWER_BYTE(OPCODE_NULL); // reset value to 0x0000 first
...
//#endif
    ...
    // [FRAME 1] Send WREG command
    // spiSendReceiveArrays(dataTx, dataRx, cmdByteLength);
    //uint16_t response = sendCommand(OPCODE_UNLOCK);

    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_8, GPIO_PIN_RESET);

    if (result != HAL_SPI_TransmitReceive(&hspi6, dataTx, dataRx, cmdByteLength, 1000)){
    	while(1);
    }
...
//!
//! \fn uint16_t calculateCRC(const uint8_t dataBytes[], uint8_t numberBytes,
//! uint16_t initialValue)
//!
//! \param dataBytes[] pointer to first element in the data byte array
//! \param numberBytes length of data byte array to include in the CRC
//! calculation \param initialValue the seed value (or partial crc calculation),
//! use 0xFFFF when beginning a new CRC computation
//!
//! NOTE: This calculation is shown as an example and has not been optimized for
//! speed.
//!
//! \return uint16_t calculated CRC word. When performing a partial CRC
//! computation, provide this value as the input to initialValue to resume
//! computing the CRC for additional input bytes.
//
//*****************************************************************************
uint16_t calculateCRC(const uint8_t dataBytes[], uint8_t numberBytes,
                      uint16_t initialValue) {
    /* Assert(s) */
    ...
    // NOTE: The polynomial's MSB is generally assumed to be high (and is
    // handled by the "dataMSb ^ crcMSb" operation below) and so it is excluded
    // from this value.
    ...
    for (byteIndex = 0; byteIndex < numberBytes; byteIndex++) {
        ...
        while (bitIndex > 0) {
            ...
            // Check if XOR operation of MSBs results in additional XOR
            // operations
            if (dataMSb ^ crcMSb) {
...
static void updateCrcIn(void) {
    // Calculate CRC word
    // NOTE: To save time, use a lookup table instead of calculating the CRC
    // value each time this function is called.
    uint16_t crcWordIn = calculateCRC(
        dataTx, ((FIXED & !CRC_MODE) ? 1 : 5) * WORD_LENGTH, 0xFFFF);
...
void restoreRegisterDefaults(void) {
    registerMap[ID_MSB_ADDRESS] =
        ID_MSB_NU_CH_4; /* NOTE: This a read-only register */
    registerMap[ID_LSB_ADDRESS] =
        0x00; /* NOTE: REV_ID value is unknown until read */
    ...
    registerMap[CLK1_ADDRESS] = CLK1_DEFAULT; //CLK1_DEFAULT;
    registerMap[CLK2_ADDRESS] = CLK2_DEFAULT; //CLK2_DEFAULT;
...
//*****************************************************************************
//
//! Internal function used by readData() to convert ADC data from multiple
//! unsigned bytes into a single signed 32-bit word.
//!
//! \fn int32_t signExtend(const uint8_t dataBytes[])
//!
//! \param dataBytes is a pointer to uint8_t[] where the first element is the
//! MSB.
//!
...
int32_t signExtend(const uint8_t dataBytes[]) {
    ...
    // NOTE: This right-shift operation on signed data maintains the signed bit,
    // and provides for the sign-extension from 24 to 32 bits.
    return (((int32_t)(upperByte | middleByte | lowerByte)) >>
            8);  // Right-shift of signed data maintains signed bit
...
void ADS131A0xXferWord(uint8_t* txData, uint8_t* rxData) {
	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_8, GPIO_PIN_RESET);
	while(hspi6.State == HAL_SPI_STATE_BUSY);
	HAL_SPI_TransmitReceive(&hspi6, txData, rxData, 4, 100); // 4 is fixed
	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_8, GPIO_PIN_SET);
}
...
uint16_t ADS131A0xReadRegister(uint8_t RegAddress) {
	...
	uint16_t word = OPCODE_RREG | (RegAddress << 8);
...
uint16_t ADS131A0xWriteRegister(uint8_t addr, uint8_t data) {
	...
	uint16_t word = OPCODE_WREG | (addr<<8) | data; // shift by 8 bits
...
//reads the device ID of the ADC
uint16_t ADS131A04xReadDeviceID(void){

	// ID_MSB_ADDRESS
	uint16_t value_msb = ADS131A0xReadRegister(ID_MSB_ADDRESS);
	uint16_t value_lsb = ADS131A0xReadRegister(ID_LSB_ADDRESS);

	return value_msb;
}

//returns a channel object containing data from all four ADC channels
adc_data_struct ADS131A0xGetChannels(void){

	adc_data_struct ch;

	ch.response = dataRx[0]<<8 | dataRx[1];
	ch.channel0 = (dataRx[4] << 16) | (dataRx[5] << 8) | (dataRx[6]);
	ch.channel1 = (dataRx[8] << 16) | (dataRx[9] << 8) | (dataRx[10]);
	ch.channel2 = (dataRx[12] << 16) | (dataRx[13] << 8) | (dataRx[14]);
	ch.channel3 = (dataRx[16] << 16) | (dataRx[17] << 8) | (dataRx[18]);
...
float ADS131A04_convert_to_mVolt(uint32_t reg)
{
    ...
    const float unitFS = 2500.0f / 8388608.0f; // unit: mV (if unit is V, calculated value is out of 'float' range)
	const uint32_t boundaryValue = 0x7FFFFF; // threshold of positive value
	//float percentage = ( float(reg) / 8388608.0f );
	int signFlg = 0;		// +: 1, -: -1

    // negative value
    if (reg > boundaryValue)
    {
        reg = (0xFFFFFF - reg) + 1; // if value is 0xFFFFFF, register is -FS/2^23. so plus 1  (2's complement)
    }
    // positive value
    else
    {
...
void StartExtADC(void) {
	//adcStartup();
	//printToConsole("ADC Init Success!");
